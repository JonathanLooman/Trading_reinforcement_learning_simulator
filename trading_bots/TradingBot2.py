...
class Trader:
    ...
    def short_open(self,price):
        '''
        Opens a short position by selling a borrowed stock and adding the short to the tracker.
        Opening the short is conditioned on capital being greater than half the price of the stock, as this half is then held as colateral in the event that the stock gains value.
        '''
    ...
    def short_close(self,price, short_num_held = 0):
        '''
        Closes the short position by purchasing a stock and returning it to the borrower.
        The profit from closing the short is the initial price of the stock minus the current price and a borrowing fee (usually around 20% per annum)
        '''
        if self.short_num>0:
            ...
            profit_amount = first_stock_held[0] - price - first_stock_held[0]*((self.time - first_stock_held[1])*(0.2/252))
            
            
            self.money += 0.5*first_stock_held[0] + profit_amount
